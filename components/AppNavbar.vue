<script lang="ts" setup>
import { umRecord } from '~~/composable/useUm'
import { useImmerseRead } from '~~/composable/useArticle'
const isImmerseRead = useImmerseRead()

// const router = useRouter()

// const { navigation } = useContent()
const goHome = () => {
  umRecord({
    eventName: 'home_logo_click',
    eventParams: {
      host: window.location.host
    }
  })
}

// const handlerChange = (item: any) => {
//   router.push(item._path)
// }
</script>

<template>
  <div
    v-show="!isImmerseRead"
    id="top-nav"
    class="sticky top-0 z-50 w-full navbar bg-base-200"
  >
    <div class="flex-1">
      <NuxtLink
        class="text-xl normal-case btn-ghost btn"
        to="/"
        @click="goHome"
      >
        <span class="uppercase">C</span>
        <span class="uppercase text-base-content">SAY</span>
        <span class="uppercase">C</span>
      </NuxtLink>
    </div>
    <div class="flex-none">
      <NuxtLink class="btn-ghost btn" to="/catogary"> 文章列表 </NuxtLink>

      <!-- <DrowpDown
        v-for="item in navigation"
        :key="item._path"
        :data="item"
        value-key="_path"
        label-key="title"
        @change="handlerChange"
        >{{ item.title }}</DrowpDown
      > -->
      <ThemeChose></ThemeChose>
      <a target="_blank" class="btn-ghost btn" href="https://github.com/zhoocoo/csayc"><svg t="1716456334104" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2593" width="24" height="24"><path d="M20.48 503.72608c0 214.4256 137.4208 396.73856 328.94976 463.6672 25.8048 6.5536 21.87264-11.8784 21.87264-24.33024v-85.07392c-148.93056 17.44896-154.86976-81.1008-164.94592-97.52576-20.23424-34.52928-67.91168-43.33568-53.69856-59.76064 33.91488-17.44896 68.48512 4.42368 108.46208 63.61088 28.95872 42.88512 85.44256 35.6352 114.15552 28.4672a138.8544 138.8544 0 0 1 38.0928-66.7648c-154.25536-27.60704-218.60352-121.77408-218.60352-233.79968 0-54.31296 17.94048-104.2432 53.0432-144.54784-22.36416-66.43712 2.08896-123.24864 5.3248-131.6864 63.81568-5.7344 130.00704 45.6704 135.168 49.68448 36.2496-9.78944 77.57824-14.9504 123.82208-14.9504 46.4896 0 88.064 5.3248 124.5184 15.23712 12.288-9.4208 73.80992-53.53472 133.12-48.128 3.15392 8.43776 27.0336 63.93856 6.02112 129.4336 35.59424 40.38656 53.69856 90.76736 53.69856 145.24416 0 112.18944-64.7168 206.4384-219.42272 233.71776a140.0832 140.0832 0 0 1 41.7792 99.9424v123.4944c0.86016 9.87136 0 19.6608 16.50688 19.6608 194.31424-65.49504 334.2336-249.15968 334.2336-465.5104C1002.57792 232.48896 782.66368 12.77952 511.5904 12.77952 240.18944 12.65664 20.48 232.40704 20.48 503.72608z" fill="#000000" opacity=".65" p-id="2594"></path></svg></a>
    </div>
  </div>
</template>
