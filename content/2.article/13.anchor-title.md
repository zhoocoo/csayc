...
- 滚动元素上设置`scroll-padding-top`值等于遮盖的高度
...
本方案实际用于`nuxt3`同构渲染中，以上为同构渲染加载图，其有很多优势，但这一条劣势导致本方案出现了这个问题。其中`FCP`即首屏时间很短，但可交互时间`TTI`变得较长。
...
既然无法改变架构，那便改变方案，采用自定义锚点，通过`scrollIntoView`方法，来进行内容锚点的定位；

- 第一步：去除正文中的`name`属性，使其不被浏览器识别为一个锚点
