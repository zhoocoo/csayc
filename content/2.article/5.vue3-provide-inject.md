...
第三点，若过多的子组件依赖于父组件的`provide/inject`，在业务开发中，业务一旦发生变化，则会导致牵一而动百，这种不可控性的业务开发过程中，使用耦合行极强的`provide/inject`。
...
首先，一部分数据是需要贯穿整个业务组件树的，我无法接收在每一个组件中，单独引入这部分数据，或者说是重新去获取数据。

其次，`vuex`这样的全局状态管理器，并不适合去存储这样的业务强相关的基础数据，后期维护，会业务全局代码混乱不堪。
